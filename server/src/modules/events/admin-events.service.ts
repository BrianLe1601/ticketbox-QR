...
export async function createAdminEvent(input:CreateAdminEventInput,adminId:number){validateTimes(input);const id=await insertAdminEvent(input,await activeCategoryId(input.category),slugify(input.name),adminId);return getAdminEventById(id);}
