...
import { cancelAdminEvent,createAdminEvent,getAdminEventById,getAdminEventList,getPublishReadiness,publishAdminEvent,removeAdminEvent,scheduleAdminEvent,updateAdminEvent } from "./admin-events.service.js";
...
export async function cancel(req:Request,res:Response,next:NextFunction){try{sendSuccess(res,await cancelAdminEvent(Number(req.params.id),(req.body as {reason:string}).reason));}catch(error){next(error);}}
